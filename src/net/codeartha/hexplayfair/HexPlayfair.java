...
import net.codeartha.hexplayfair.PairPointReturn;
...
public class HexPlayfair {
	...
	private static PairPointReturn decryptCoords(Point cryptedA, Point cryptedB) {
		...
		{
			clearA.x = cryptedA.x;
			clearA.y = (cryptedA.y + References.GRID_SIZE - 1) % References.GRID_SIZE; // +4 mod(5) to solve the issue with removing 1 that sometimes gives negative numbers instead of looping around
			clearB.x = cryptedB.x;
